...
public sealed class SetAvatarCommandHandler : ICommandHandler<SetAvatarCommand>
{
    ...
    public async Task<Result> Handle(SetAvatarCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Failure(Error.Validation("Media.InvalidKind", "An avatar must be an image."));
        }
 
        if (asset.Category != MediaCategory.User)
        {
            return Result.Failure(Error.Validation("Media.InvalidCategory", "Media must be uploaded under the User category to be used as an avatar."));
        }
        ...
        // Deliberate: the previous avatar's MediaAsset row is left exactly as
        // it was (kept as history, per Phase 5 decision) — only the pointer
        // moves.
        profile.SetAvatar(asset.Id);
