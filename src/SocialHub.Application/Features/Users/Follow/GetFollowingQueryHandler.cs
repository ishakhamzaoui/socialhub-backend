...
public sealed class GetFollowingQueryHandler : IQueryHandler<GetFollowingQuery, PagedUserListDto>
{
    ...
    public async Task<Result<PagedUserListDto>> Handle(GetFollowingQuery request, CancellationToken cancellationToken)
    {
        ...
        switch (access)
        {
            ...
            case ProfileAccessResult.Denied:
                return Result.Failure<PagedUserListDto>(Error.Forbidden("Profile.Private", "This user's following list is not visible to you."));
        }
 
        var excludeIds = await GetBlockedRelationshipIdsAsync(requesterId, cancellationToken);
        var (ids, total) = await _followRepository.GetFollowingIdsAsync(request.UserId, request.Page, request.PageSize, excludeIds, cancellationToken);
